layui.define([], function (exports){
	...
	tree.prototype.template = function (e) {
		var t = this,
			level = [],
			tbody = '',
			is_table = $('table' + e.elem).length || !(e.is_click_icon = true),
			...
			thead = checkbox && '<th style="width:28px;">' + ($.inArray(e.top_value, e.checked.data) > -1 ? checked : checkbox) + '</th>';
		$.each(t.data(e, e.data), function (idx, item) {
			var tr = '',
				is_checked = false,
			...
			$.each(e.cols, function (index, obj) {
				var style = '';
				obj.width && (style += 'width:' + obj.width + ';'), obj.align && (style += 'text-align:' + obj.align + ';'), style && (style = 'style="' + style + '"');
				// 标记设置行checkbox选中
				if (e.is_checkbox && e.checked && $.inArray(item[e.checked.key], e.checked.data) > -1) {
					is_checked = true;
				}
				// 第一次遍历头部的时候拼接表格头部
		...
		// 模板渲染并处理点击展开收起等功能
		$(e.elem).html(table).off('click', click_btn).on('click', click_btn, function () {
			var tr = $(this).parents('[data-id]'),
			...
			var is_checked = $(this).toggleClass('layui-form-checked').hasClass('layui-form-checked'),
				tr = $(this).parents('[data-id]'),
				id = tr.data('id'),
				pid = tr.data('pid');
	...
	tree.prototype.parents_checkbox = function (e, pid) {
		var po = $(e.elem).find('[data-pid=' + pid + ']'),
			co = $(e.elem).find('[data-id=' + pid + ']'),
			len = $(e.elem).find('[data-pid=' + pid + '] .cbx.layui-form-checked').length;
	...
	tree.prototype.childs_checkbox = function (e, id, is_checked) {
		var t = this;
		$(e.elem).find('[data-pid=' + id + ']').each(function () {
			var checkbox = $(this).find('.cbx');
			is_checked ? checkbox.addClass('layui-form-checked') : checkbox.removeClass('layui-form-checked');
			t.childs_checkbox(e, $(this).data('id'), is_checked);
		})
	};
	// 点击收起循环隐藏子级元素
	...
	tree.prototype.checked = function (e) {
		var ids = [];
		$(e.elem).find('.cbx.layui-form-checked').each(function () {
